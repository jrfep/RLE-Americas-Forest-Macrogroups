...
for (k in paste("bsq",2001:2012,sep=""))
    system(sprintf("r.stats -a -c input=SouthAmerica_IVC_MacroGroups_potential_NatureServe_v7_270m,Paises,AOO,%s output=calculos_201712/SAM_Paises_AOO_%s.tab fs=space",k,k))

for (k in sprintf("Anthromes_%s00",17:20))
    system(sprintf("r.stats -a -c input=SouthAmerica_IVC_MacroGroups_potential_NatureServe_v7_270m,Paises,AOO,%s output=calculos_201712/SAM_Paises_AOO_%s.tab fs=space",k,k))

for (k in c("RS","GWSchange")[2])
    system(sprintf("r.stats -a -c input=SouthAmerica_IVC_MacroGroups_potential_NatureServe_v7_270m,Paises,%s output=calculos_201712/SAM_Paises_%s.tab fs=space ",k,k))


  system(sprintf("r.stats -a -c input=SouthAmerica_IVC_MacroGroups_potential_NatureServe_v7_270m,Paises,Anthromes_1700,Anthromes_1800,Anthromes_1900,Anthromes_2000 output=calculos_201712/SAM_Paises_Anthromes_Changes.tab fs=space"))
...
for (frmt in c("1.A.1","1.A.2","1.A.3","1.A.4","1.A.5","1.B.1","1.B.2","1.B.3")) {
    ...
    for (mcdg in cdgs[cdgs %in% subset(tipologia,format %in% frmt & macrogroup_key != "")$macrogroup_key]) {
        ...
        tmp <- read.table("calculos_201712/SAM_Paises_GWSchange.tab")
        ...
        for (l in ls) {
            system(sprintf("r.mapcalc 'tmp=if(SouthAmerica_IVC_MacroGroups_potential_NatureServe_v7_270m@PERMANENT==%s & Paises==%s & bsq2012>24,1,null())'",cdg,l))
