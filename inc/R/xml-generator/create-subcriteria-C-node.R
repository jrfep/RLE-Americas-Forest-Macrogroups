...
C2a.rationale <- with(assess.fun,
  rD(assess.total$C2a,best.estimate.mean.severity.ClimateChange.2000.2050,
    ...
    extent.with.severity.30.or.higher.ClimateChange.2000.2050,
      extent.with.severity.50.or.higher.ClimateChange.2000.2050,
        extent.with.severity.80.or.higher.ClimateChange.2000.2050)
)
...
if (assess.total$C2a %in% "DD") {
   C2a.IndValues <- newXMLNode("Values","Model performance on current data was not good enough, and no future predictions were attempted")
} else {
   C2a.IndValues <- newXMLNode("Values",children=list(
    newXMLNode("Value",attrs=list(method="mean estimated"),children=list(
        newXMLNode("Severity",attrs=list(units="%"), best.estimate.mean.severity.ClimateChange.2000.2050),
...
C2a <- newXMLNode("Subcriterion",attrs=list(name="C2a"),
      children=list(newXMLNode("Summaries",children=list(C2a.summary)),
        ...
        newXMLNode("Bounds",ifelse(is.na(assess.total$bounds.C2a),"",assess.total$bounds.C2a)),
        C2a.variable))
...
C2b.rationale <- with(assess.fun,
  rD(assess.total$C2b,best.estimate.mean.severity.SurfaceWater.19XX.20XX0,
     NA,
    extent.with.severity.30.or.higher.SurfaceWater.19XX.20XX,
      extent.with.severity.50.or.higher.SurfaceWater.19XX.20XX,
        extent.with.severity.80.or.higher.SurfaceWater.19XX.20XX)
)
         ...
         children=list(newXMLNode("Key-indicator",
           children=list(newXMLNode("Indicator-data",
            ...
            C2b.IndValues,
             ...
             newXMLNode("Overall-Extent",attrs=list(units="%",method="lower-upper"), with(assess.fun,getOExt(assess.total$C2b,
               extent.with.severity.30.or.higher.Surface.Water.19XX.20XX,
                 extent.with.severity.50.or.higher.Surface.Water.19XX.20XX,
                   extent.with.severity.80.or.higher.Surface.Water.19XX.20XX))),
           newXMLNode("Collapse-threshold","We set 85% decline in surface water as the collapse threshold."))))
        ) # Indicator data
     ) # Key indicator
   ) # Key indicator variable
   ) # with
