...
D1.variable <- with(assess.fun,newXMLNode("Key-indicator-variable",attrs=list(name="Resource use intensity"),
  children=list(newXMLNode("Key-indicator",
    ...
    D2b.variable <- with(assess.fun,newXMLNode("Key-indicator-variable",attrs=list(name="Potential defaunation"),
      children=list(newXMLNode("Key-indicator",
        children=list(newXMLNode("Indicator-data",
          children=list(
             newXMLNode("Data-source","Benítez-López, A., Alkemade, J.R.M., Schipper, A.M., Ingram, D.J., Verweij, P.A., Eikelboom, J. & Huijbregts, M. (2017). The impact of hunting on tropical mammal and bird populations. Science (80-. )., 356, 180–183."),
             newXMLNode("Data-source","Digital Chart of the World: road network per country"),
             newXMLNode("Data-source","Friedl, M.A., Sulla-Menashe, D., Tan, B., Schneider, A., Ramankutty, N., Sibley, A. & Huang, X. (2010). MODIS Collection 5 global land cover: Algorithm refinements and characterization of new datasets, 2001-2012, Collection 5.1 IGBP Land Cover."),
             newXMLNode("Data-source","World Database on Protected Areas, IUCN and UNEP-WCMC 2016"),
         newXMLNode("Data-description","We applied a simplified formula relating response ratios (RR) of the abundance of mammals in hunted and unhunted sites with three variables: distance to access points for hunters (approximated by distance in km to the  main road network, from Digital Chart of the World), distance to markets for wildlife products (approximated by the distance in km to major urban centers, calculated from urban land cover in MODIS LC, version 5.1, Friedl et al. 2010)⁠ and level of protection (approximated by the distribution of any kind of protected areas, from the World Database on Protected Areas, IUCN and UNEP-WCMC 2016). According to figures and detailed results from (Benítez-López et al. 2017)⁠⁠, we used the following formula to estimate the expected RR from raster images representing the three variables: RR = -2.370 + 0.057 access + 0.010 market + 1.270 protected . We then transformed the spatial estimate of RR to an estimate of potential decline in mammal abundance as a percentage . For negative RR values we estimated the decline as: decline = (1-exp(RR)) * 100. For zero or positive RR values we assumed zero decline. An overall weighted estimate of percentage decline was calculated over the potential distribution of each Macrogroup, using mean forest cover between 2001 and 2012 as a weighting variable. We assume that the applied formula represents the potential relative decline in mammal abundance and used a collapse threshold equivalent of 10% of the original abundance. We further assume that the formula represents a current level of threat representative a 50 year window including past, present and future."),
          newXMLNode("Values",children=list(
            newXMLNode("Value",attrs=list(method="mean estimated"),children=list(
                newXMLNode("Severity",attrs=list(units="%"), best.estimate.mean.severity.Defaunation.19XX.20XX),
                newXMLNode("Extent",attrs=list(units="%"),'100')
            )),
          newXMLNode("Value",attrs=list(method="estimated"),children=list(
                newXMLNode("Severity",attrs=list(units="%"),">30"),
                newXMLNode("Extent",attrs=list(units="%"),extent.with.severity.30.or.higher.Defaunation.19XX.20XX)
            )),
            newXMLNode("Value",attrs=list(method="estimated"),children=list(
                newXMLNode("Severity",attrs=list(units="%"),">50"),
                newXMLNode("Extent",attrs=list(units="%"),extent.with.severity.50.or.higher.Defaunation.19XX.20XX)
            )),
            newXMLNode("Value",attrs=list(method="estimated"),children=list(
                newXMLNode("Severity",attrs=list(units="%"),">80"),
                newXMLNode("Extent",attrs=list(units="%"),extent.with.severity.80.or.higher.Defaunation.19XX.20XX)
            ))
         ))
          ,
          newXMLNode("Overall-Severity",attrs=list(units="%",method="lower-upper"), with(assess.fun,getOSev(as.character(D2b),
            extent.with.severity.30.or.higher.Defaunation.19XX.20XX,
              extent.with.severity.50.or.higher.Defaunation.19XX.20XX,
                extent.with.severity.80.or.higher.Defaunation.19XX.20XX))),
          newXMLNode("Overall-Extent",attrs=list(units="%",method="lower-upper"), with(assess.fun,getOExt(as.character(D2b),
            extent.with.severity.30.or.higher.Defaunation.19XX.20XX,
              extent.with.severity.50.or.higher.Defaunation.19XX.20XX,
                extent.with.severity.80.or.higher.Defaunation.19XX.20XX))),
        newXMLNode("Collapse-threshold","We considered a ecosystem collapsed when the mammal abundance declined to 10% of the original abundance."))))
     ) # Indicator data
  ) # Key indicator
