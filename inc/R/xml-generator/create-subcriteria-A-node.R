...
A1.variable <- with(assess.spa,newXMLNode("Key-indicator-variable",attrs=list(name="Resource use intensity"),
...
A3.variable <- with(assess.spa,newXMLNode("Key-indicator-variable",attrs=list(name="Resource use intensity"),
children=list(newXMLNode("Key-indicator",
  children=list(newXMLNode("Indicator-data",
    children=list(newXMLNode("Data-source","Ellis, E.C., Goldewijk, K.K., Siebert, S., Lightman, D. & Ramankutty, N. (2010). Anthropogenic transformation of the biomes, 1700 to 2000. Glob. Ecol. Biogeogr., 19, 589–606."),
    ...
    newXMLNode("Values",children=list(
      newXMLNode("Value",attrs=list(method="decline"),children=list(
          newXMLNode("Indicator-value",attrs=list(units="%"), best.estimate.decline.1750.2000)
      )),
      newXMLNode("Collapse-threshold","The macrogroup was considered collapse when woodland cover declined to 0 square km (100% decline)."))
   ))))))))
   ...
   A3 <- newXMLNode("Subcriterion",attrs=list(name="A3"),
         children=list(newXMLNode("Summaries",children=list(A3.summary)),
           ...
           newXMLNode("Bounds",ifelse(is.na(assess.total$bounds.A3),"",assess.total$bounds.A3)),
           A3.variable))
...
children=list(newXMLNode("Summaries",children=list(A2b.summary)),
 newXMLNode("Rationale",A2b.rationale),
 newXMLNode("Category",assess.total$A2b),
 newXMLNode("Bounds",ifelse(is.na(assess.total$bounds.A2b),"",assess.total$bounds.A2b)),
 A2b.variable))
